        ...
        fun loadUsers(cfg: CFGSection) {
            for ((idString, cfgUser) in cfg.sections) {
                ...
                try {
                    ...
                    System.err.println("Invalid UUID for permission user: $idString")
                    continue
                }
        ...
        fun loadGroups(cfg: CFGSection) {
            for ((idString, cfgGroup) in cfg.sections) {
                ...
                try {
                    ...
                    System.err.println("Invalid UUID for permission group: $idString")
                    continue
                }
                ...
                if (cfgGroup.isList("members")) {
                    for (userIdString in cfgGroup.getStringList("members")) {
                        ...
                        try {
                            ...
                            System.err.println("Invalid UUID for group member: (group: ${group.name}, id: $userIdString)")
                            continue
                        }
                        val user = usersById[userId]
                        if (user == null) {
                            System.err.println("User does not exist: (group: ${group.name}, id: $userId)")
                            continue
                        }
