...
import com.asledgehammer.framework.event.command.*
...
object Events {
    ...
    fun dispatch(event: Event) {
        ...
        for (wrapper in wrappers) {
            try {
                ...
                System.err.println("Failed to execute listener: ${clazz.simpleName}. Disabling listener.")
    ...
    fun dispatch(command: Command, commander: Commander): CommandExecution {
        ...
        for (wrapper in wrappers) {
            try {
                ...
                System.err.println("Failed to execute listener: ${wrapper.javaClass.simpleName}. Disabling.")
                throwable.printStackTrace(System.err)
                wrapper.isEnabled = false
    ...
    private class Wrapper {
        ...
        private fun getAllDeclaredMethods(clazz: Class<*>): List<Method> {
            val methods = java.util.ArrayList<Method>()
            var c: Class<*>? = clazz
            while (c != null) {
